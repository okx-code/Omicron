...
public class LoggingListener extends ListenerAdapter {
    private final BlockingQueue<CachedMessage> messageCache = new ArrayBlockingQueue<>(128);
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if(messageCache.remainingCapacity() < 1) {
            messageCache.poll();
        }
        messageCache.offer(new CachedMessage(e.getMessageIdLong(), e.getMessage().getContentRaw(), e.getMessage().getCreationTime()));
    }
    ...
    public void onGuildMessageUpdate(GuildMessageUpdateEvent e) {
        omicron.getLoggingManager().isValid(e.getGuild(), e.getChannel(), e.getClass())
                .thenAccept(r -> {
                    ...
                    messageCache.forEach(cachedMessage -> {
                        if(cachedMessage.getId() == e.getMessageIdLong()) {
                            eb.addField("Old Message", cachedMessage.getOldContent(), false);
                            cachedMessage.setOldContent(e.getMessage().getContentRaw());
                        }
                    });
    ...
    public void onGuildMessageDelete(GuildMessageDeleteEvent e) {
        omicron.getLoggingManager().isValid(e.getGuild(), e.getChannel(), e.getClass())
                .thenAccept(r -> {
                    ...
                    messageCache.forEach(cachedMessage -> {
                        if(cachedMessage.getId() == e.getMessageIdLong()) {
                            eb.addField("Message", cachedMessage.getOldContent(), false);
